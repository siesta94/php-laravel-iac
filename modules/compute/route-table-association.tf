...
resource "aws_route_table_association" "pm4_web_rt_association_a" {
  subnet_id      = aws_subnet.pm4_web_a.id
  route_table_id = aws_route_table.pm4_web_a.id
}

resource "aws_route_table_association" "pm4_web_rt_association_b" {
  subnet_id      = aws_subnet.pm4_web_b.id
  route_table_id = aws_route_table.pm4_web_b.id
}
#TASKS
